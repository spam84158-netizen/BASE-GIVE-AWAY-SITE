...
router.get('/rewards', (req, res) => {
  ...
  const rewards = data.rewards
    ...
    .map((r) => ({
      ...
      participantsCount: data.participants.filter((p) => p.rewardId === r.id && p.status !== 'blocked').length,
    }));
  res.json(rewards);
});
...
router.get('/rewards/:id', (req, res) => {
  const data = db.read((d) => d);
  const reward = data.rewards.find((r) => r.id === req.params.id && r.status !== 'disabled');
  if (!reward) return res.status(404).json({ error: 'Récompense introuvable.' });
  res.json({
    ...publicReward(reward),
    referralBonus: data.settings.referralBonusAmount,
    participantsCount: data.participants.filter((p) => p.rewardId === reward.id && p.status !== 'blocked').length,
  });
});
